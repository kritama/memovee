# Handler foundation; issue #12 supplies the executable chain.

module "remember-retry-save-request" {
  source      = "upmaru/base/tama//modules/forwardable-class"
  version     = "0.5.6"
  space_id    = tama_space.memory-write.id
  title       = "remember-retry-save-request"
  description = "Memory v1 remember-retry-save handoff; preserves origin, concept and directive provenance."
}

resource "tama_chain" "remember-retry-save" {
  space_id = tama_space.memory-write.id
  name     = "remember-retry-save"
}

# Keep the handler disabled until its terminal paths are implemented.
resource "tama_node" "remember-retry-save" {
  count    = 0
  space_id = tama_space.memory-write.id
  class_id = module.remember-retry-save-request.class.id
  chain_id = tama_chain.remember-retry-save.id
  type     = "reactive"
  on       = "processing"
}
