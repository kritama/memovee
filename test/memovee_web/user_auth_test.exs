defmodule MemoveeWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, MemoveeWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      MemoveeWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_scope_for_user/2" do
    test "authenticates user from session", %{conn: conn, user: user} do
      ...
      assert conn.assigns.current_scope.actor.id == user.actor.id

      assert DateTime.compare(
               conn.assigns.current_scope.user.authenticated_at,
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        MemoveeWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
